...
import { buildPageElement as _buildPageElement } from "${rscRuntimeDir}/page-builder.js";
import { executeRouteHandler as _executeRouteHandler } from "${rscRuntimeDir}/route-handler.js";
...
// buildPageElement — imported from rsc-runtime/page-builder
const __pageCtx = {
  rootNotFoundComponent: ${rootNotFoundVar ? `${rootNotFoundVar}?.default ?? null` : "null"},
  globalErrorComponent: ${globalErrorVar ? `${globalErrorVar}?.default ?? null` : "null"},
};
function buildPageElement(route, params, opts, searchParams) {
  return _buildPageElement(route, params, opts, searchParams, __pageCtx);
}
...
async function _handleRequest(request, __reqCtx) {
  ...
  const __routeHandlerResponse = await _executeRouteHandler(
    route, request, params, cleanPathname,
    () => { setHeadersContext(null); setNavigationContext(null); },
  );
  if (__routeHandlerResponse) return __routeHandlerResponse;
