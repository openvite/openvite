/**
 * Pages Router entry generation — extracted from the original index.ts.
 *
 * These functions generate the virtual server and client entry modules
 * for the Pages Router production build.
 */
import type { PluginContext } from "../core/plugin-context.js";
import { pagesRouter, apiRouter, patternToNextFormat as pagesPatternToNextFormat, type Route } from "../routing/pages-router.js";
 ...
 */
export async function generateServerEntry(ctx: PluginContext): Promise<string> {
  const pageRoutes = await pagesRouter(ctx.pagesDir);
  ...
  const hasApp = fs.existsSync(path.join(ctx.pagesDir, "_app.tsx")) || fs.existsSync(path.join(ctx.pagesDir, "_app.jsx")) || fs.existsSync(path.join(ctx.pagesDir, "_app.ts")) || fs.existsSync(path.join(ctx.pagesDir, "_app.js"));
  const hasDoc = fs.existsSync(path.join(ctx.pagesDir, "_document.tsx")) || fs.existsSync(path.join(ctx.pagesDir, "_document.jsx")) || fs.existsSync(path.join(ctx.pagesDir, "_document.ts")) || fs.existsSync(path.join(ctx.pagesDir, "_document.js"));

  // Use absolute paths for _app and _document too
  const appFileBase = path.join(ctx.pagesDir, "_app").replace(/\\/g, "/");
  const docFileBase = path.join(ctx.pagesDir, "_document").replace(/\\/g, "/");
  ...
  const i18nConfigJson = ctx.nextConfig?.i18n
    ? JSON.stringify({
        locales: ctx.nextConfig.i18n.locales,
        defaultLocale: ctx.nextConfig.i18n.defaultLocale,
        localeDetection: ctx.nextConfig.i18n.localeDetection,
      })
    : "null";
  ...
  const openviteConfigJson = JSON.stringify({
    basePath: ctx.nextConfig?.basePath ?? "",
    trailingSlash: ctx.nextConfig?.trailingSlash ?? false,
    redirects: ctx.nextConfig?.redirects ?? [],
    rewrites: ctx.nextConfig?.rewrites ?? { beforeFiles: [], afterFiles: [], fallback: [] },
    headers: ctx.nextConfig?.headers ?? [],
    i18n: ctx.nextConfig?.i18n ?? null,
    images: {
      deviceSizes: ctx.nextConfig?.images?.deviceSizes,
      imageSizes: ctx.nextConfig?.images?.imageSizes,
  ...
  const middlewareImportCode = ctx.middlewarePath
    ? `import * as middlewareModule from ${JSON.stringify(ctx.middlewarePath.replace(/\\/g, "/"))};
import { NextRequest } from "next/server";`
  ...
  const middlewareExportCode = ctx.middlewarePath
    ? `
...
var isProxy = ${ctx.middlewarePath ? JSON.stringify(isProxyFile(ctx.middlewarePath)) : "false"};
 ...
 */
export async function generateClientEntry(ctx: PluginContext): Promise<string> {
  const pageRoutes = await pagesRouter(ctx.pagesDir);
